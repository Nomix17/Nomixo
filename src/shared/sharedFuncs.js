...
const xRemoveIcon = `            
  <svg class="closeButtonIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
...
function getMovieDetailPageCacheData() {
  const serieEpisodesDiv = document.getElementById("EpisodesContainer"); 
  ...
  const episodes = document.querySelectorAll(".div-episodes-Element");
  const loadedEpisode = [...episodes].findIndex(el => el.classList.contains("pressed_episode"));
  ...
  const cacheData = {
    ...
    "loaded_episode": loadedEpisode + 1
  };
...
async function handleFullScreenIcon() {
  const isFullscreened = window.__initialNavState.isFullscreened;
  changeFullscreenIcon(isFullscreened);
}

function changeFullscreenIcon(isFullscreened) {
...
async function fullscreenClicked(){
  ...
  changeFullscreenIcon(isFullScreen);
}
