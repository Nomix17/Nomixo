...
function openMediaVideo(MediaId,MediaType,MagnetLink,IMDB_ID,backgroundImage,episodeInfo){
  ...
  let path = `./videoPlayer/videoPlayer.html?MagnetLink=${b64MagnetLink}&id=${IMDB_ID}&MediaId=${MediaId}&MediaType=${MediaType}&episodeNumber=${episodeInfo?.episodeNumber}&seasonNumber=${episodeInfo?.seasonNumber}&bgPath=${backgroundImage}`;
  window.electronAPI.navigateTo(path); 
}
...
function addContrastForPlayIcon(){
  ...
  const bgColor = luminance > 180 ? "rgba(25,25,25,0.7)" : "#f9f9f9";
