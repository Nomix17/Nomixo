...
function insertTorrentInfoElement(data,MediaId,MediaType,MediaLibraryInfo,episodeInfo={}){
    ...
    TorrentResutls.forEach(element => {
      ...
      if(parseInt(SeedersNumber)){
        ...
        if(MediaLibraryInfo?.typeOfSave?.includes("Currently Watching") && String(MagnetLink) == String(MediaLibraryInfo["Magnet"]) && String(episodeInfo.seasonNumber) == String(MediaLibraryInfo["seasonNumber"]) && String(episodeInfo.episodeNumber) == String(MediaLibraryInfo["episodeNumber"]))
